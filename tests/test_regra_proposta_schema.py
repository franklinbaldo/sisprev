...
def _unidade(
    doc_id: str, *, origens_legacy: list[str], estado_proposta: str = "elaboracao", **extra: object
) -> RegraProposta:
    frontmatter: dict[str, object] = {
        "type": "RegraProposta",
        "id": doc_id,
        "schema_version": 1,
        "estado_proposta": estado_proposta,
        "origens_legacy": origens_legacy,
        **extra,
    }
    return RegraProposta(doc_id=doc_id, frontmatter=frontmatter)
...
def test_empty_bundle_dir_loads_as_no_units(tmp_path: Path) -> None:
    ...
    assert load_regras_propostas(tmp_path) == []
...
def test_duplicate_id_is_flagged() -> None:
    ...
    violations = validate_bundle_proposto(unidades, _legacy_bundle("regra-0022"))
    assert any(v.code == "PROPOSTA_ID_DUPLICADO" for v in violations)
...
def test_unknown_origin_is_flagged() -> None:
    ...
    violations = validate_bundle_proposto(unidades, _legacy_bundle("regra-0001"))
    assert any(v.code == "PROPOSTA_ORIGEM_INEXISTENTE" for v in violations)
...
def test_one_to_n_decomposition_is_valid() -> None:
    ...
    assert validate_bundle_proposto(unidades, _legacy_bundle("regra-0022")) == []
...
def test_n_to_one_consolidation_is_valid() -> None:
    ...
    assert validate_bundle_proposto(unidades, _legacy_bundle("regra-0001", "regra-0002")) == []
...
def test_faixa_exposicao_distinguishes_one_to_n_units() -> None:
    ...
    assert validate_bundle_proposto(unidades, _legacy_bundle("regra-0071")) == []
...
def test_id_is_independent_of_row_index() -> None:
    ...
    assert validate_bundle_proposto([unidade], _legacy_bundle("regra-0022")) == []
...
def test_id_shaped_like_a_legacy_regra_id_is_rejected() -> None:
    ...
    violations = validate_bundle_proposto(unidades, _legacy_bundle("regra-0022"))
    assert any(v.code == "PROPOSTA_ID_INVALIDO" for v in violations)
...
def test_malformed_frontmatter_is_flagged_without_hiding_identity_errors() -> None:
    ...
    violations = validate_bundle_proposto([unidade], _legacy_bundle("regra-0001"))
    codes = {v.code for v in violations}
    assert "PROPOSTA_FRONTMATTER_INVALIDA" in codes
    assert "PROPOSTA_ORIGEM_INEXISTENTE" in codes
