...
O achado é `improcedente` não porque a colisão observada seja falsa — as
oito regras legadas realmente compartilham `tipo_calculo: Proporcionalidade Dias` entre benefícios distintos, e essa constatação continua registrada,
como nota de proveniência, em `tipo-calculo-media-proporcional-dias-lce1100`
(o `TipoCalculo` que sucedeu a `FormaCalculo` citada acima) — mas porque a
**classificação como achado bloqueante da auditoria estava errada**. Ela
tratava como pendência jurídica da derivação uma pergunta que é de
implantação: como o Sisprev, sistema-alvo, representa internamente a
fórmula já corretamente derivada. A primeira pergunta é trabalho da
auditoria; a segunda não é, e incerteza sobre a segunda não deveria manter
uma derivação já concluída em estado de pendência jurídica.

A pendência real — confirmar que o rótulo `Proporcionalidade Dias` não
ambiguiza a rotina de cálculo executada no Sisprev para a causa comum da
LCE 1.100/2021 — está registrada em `estado_implantacao: pendente_mapeamento_sisprev` nas duas `RegraProposta` correspondentes
(`incapacidade-lce1100-ate-2003-causa-comum`,
`incapacidade-lce1100-apos-2003-causa-comum`), e acompanhada pelas issues
#122 e #124. A § "Efeito" acima cita `estado_proposta` e `estado_grupo`,
campos que existiam quando este achado foi registrado; ambos foram
substituídos por `estado_auditoria` e pela atomicidade de implantação
derivada por componentes conexos sobre `origens_legacy` — o efeito prático
que a seção descrevia (as duas regras não entram na carga de implantação
enquanto a confirmação não chega) permanece o mesmo sob os nomes atuais.
