...
ressalva_homologacao: >-
  Duas conferências, materialmente distintas, incidem sobre esta unidade. **Sujeição ao
  regime complementar e teto do RGPS:** confirmar em homologação a forma pela qual o
  Sisprev identifica a sujeição e aplica o limite máximo dos benefícios do RGPS (art.
  24, §§ 11 e 12) — os dados disponíveis não revelam em qual etapa esse limite incide
  nem por qual informação o sistema reconhece a sujeição, e, até haver seleção
  automática suficiente, a data de ingresso e a eventual opção prévia e expressa devem
  ser verificadas manualmente na instrução, antes da seleção da regra e do ato
  concessório. **Base da proporcionalização:** confirmar que `Proporcionalidade Dias`,
  nesta hipótese, executa a fração do art. 26 sobre a média do art. 24 já limitada pelo
  § 10 e pelo teto do RGPS, nessa ordem, e não uma proporcionalidade nua sobre a
  remuneração. Há evidência operacional de que o motor executa a base, porque
  `regra-0021` já grava `Proporcionalidade Dias` em produção para população que inclui
  quem ingressou a partir de 06/11/2018. Divergência apurada em qualquer das duas
  impede a ativação em produção e retorna para decisão técnica ou jurídica, conforme a
  natureza.
origens_legacy:
  - regra-0021
predicados:
