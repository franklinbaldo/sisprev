...
type: Achado
id: achado-0008
nome: 'Pendência de Preenchimento: Regras Ativas com Campos Obrigatórios Vazios'
situacao: aberto
...
As regras citadas estão marcadas como ativas no sistema, mas possuem campos estruturais essenciais vazios ou marcados como 'Não identificado', particularmente `sexo`, `integral` ou `tipo_calculo`.

# Evidências

A detecção `P9_CAMPOS_VAZIOS_PENDENTES` encontrou estas regras com valores vazios nos campos mencionados. Uma regra ativa não pode ter indefinições nesses campos fundamentais.

# Questão a investigar

Investigar e completar os dados faltantes nas regras afetadas, determinando o `sexo` correto (AMBOS, MASCULINO, FEMININO), flag de `integral` e o `tipo_calculo` correspondente a partir das fontes normativas.
