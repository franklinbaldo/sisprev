...
| 5   | **Markov com estado aumentado** (acumuladores como coordenadas do próprio estado)                                                             | Sim, **por construção** — ver prova informal (§2.2)                                                                                                                                                                                                                                         | **Escolhida como kernel principal** (§2.1)                                                       |
...
de estado (§5.1): idade (derivável de nascimento + instante, mas mantida
...
ocorrência da incapacidade, e a versão temporal da taxonomia vigente em cada
data relevante. **A granularidade inicial é mensal** — fina o bastante para
...
### 2.2 Por que o estado aumentado preserva a propriedade de Markov (prova informal)
...
histórico completo. O vetor de estado factual (§5.1) inclui, como
**coordenadas explícitas**, exatamente as quantidades cuja dependência do
histórico a missão desta RFC identificou como memória: idade, tempo de
contribuição, tempo de serviço público, duração da incapacidade, vigência
normativa aplicável. **Cada uma dessas quantidades é, por definição, uma
estatística suficiente do histórico relevante para prever a transição
seguinte** — não é preciso saber *quando exatamente* cada mês de
contribuição ocorreu, só *quanto* acumulou até agora, porque as regras
jurídicas (e portanto os kernels sintéticos que as testam) só usam a
quantidade acumulada, nunca a trajetória detalhada de como ela foi
composta. Ao colocar esses acumuladores **no próprio estado** (em vez de
deixá-los implícitos no histórico), a transição `estado(t) → estado(t+1)`
passa a ser função só de `estado(t)` — a definição de Markov é satisfeita
**por construção**, não por presunção.

O preço é um espaço de estados maior (cada acumulador multiplica a
cardinalidade) — mitigado pela granularidade mensal e por horizontes
temporais limitados (uma questão em aberto, §19).

### 2.3 Quando semi-Markov seria necessário — não descartado, deixado em aberto

A duração da incapacidade **já é um acumulador do estado aumentado**
(§5.1) — logo, uma probabilidade de transição que dependa de "quantos meses
já se passaram desde a ocorrência da incapacidade sem perícia" **continua
sendo Markoviana** sobre o estado aumentado (a dependência é do *valor
atual* do acumulador, não do histórico bruto). Isso cobre boa parte do que
um leitor apressado chamaria de "precisa de semi-Markov" — mas não cobre
tudo:
...
| `data_ingresso_servico_publico`   | marco de ingresso no serviço público                                                                                                                                                                                                                                                                                                             | usado para `regime`/`marco_ingresso` (RFC 0004 §3)                                                                                                                              |
...
| `tempo_contribuicao_acumulado`    | meses de contribuição efetivamente computados                                                                                                                                                                                                                                                                                                    | **acumulador** — só cresce enquanto `vinculo_regime` está em estado que conta como contribuição; ver invariante de monotonicidade (§6.2)                                        |
...
| `vinculo_regime`                  | vínculo/regime previdenciário corrente (enum sintético, referencia os regimes já catalogados: pré-EC20, EC20, EC41+LCE432, EC103+LC1100, ...)                                                                                                                                                                                                    | nunca inferido de `nome`/`fundamentacao*` (gate §17)                                                                                                                            |
...
| `duracao_incapacidade_meses`      | acumulador: meses desde `data_ocorrencia_incapacidade` sem constatação concreta                                                                                                                                                                                                                                                                  | zera não é permitido — só cresce até a constatação (§6.2)                                                                                                                       |
...
| `nexo_acidente_servico`           | avaliação do requisito específico (RFC 0004 §7, parte 5): `satisfeito`/`não satisfeito`/`indeterminado`                                                                                                                                                                                                                                          | sempre acompanhado de `base_avaliacao` (§7)                                                                                                                                     |
| `nexo_molestia_profissional`      | idem, para o requisito equivalente — **nota**: a base normativa (§3.3.3 do documento irmão) já registra que nenhum dos dois regimes estaduais lidos define "moléstia profissional" (P-6); a simulação pode gerar este predicado como `indeterminado`/`sem_informacao` estruturalmente, sem que isso implique que a lacuna jurídica foi resolvida | mesmo tratamento de `base_avaliacao`                                                                                                                                            |
| `enquadramento_rol_doenca`        | avaliação do requisito "doença enquadrada no rol aplicável"                                                                                                                                                                                                                                                                                      | idem                                                                                                                                                                            |
| `versao_rol_vigente`              | qual versão da taxonomia de doenças estava em vigor no instante relevante (LCE 432/2008: 14 doenças; LCE 1.100/2021: 16) — o próprio "Q6-T-vigência" de RFC 0004 §16.2                                                                                                                                                                           | nunca resolvido por presunção; a simulação pode deliberadamente gerar cenários com essa versão `pendente` para testar o comportamento fail-closed do compilador (RFC 0004 §5.3) |
| `fatos_verificacao_humana`        | lista de `{predicado, base_avaliacao, avaliacao, responsavel?, data?, referencia?}` — espelha as cinco partes de RFC 0004 §7                                                                                                                                                                                                                     | ver §7 desta RFC                                                                                                                                                                |
| `contradicoes_e_dados_ausentes`   | lista estruturada de contradições detectadas dentro do próprio estado (p.ex. `nexo_acidente_servico = satisfeito` mas `classe_causa = causa_comum`)                                                                                                                                                                                              | usada pela estratégia de geração de contradições deliberadas (§10)                                                                                                              |
...
| Transição                           | Efeito no estado                                                                                                                                                                                                                                                         | Pré-condição                                                                                                                                        |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| passagem do tempo                   | `instante_simulado += 1` (mês); `tempo_contribuicao_acumulado`/`tempo_servico_publico_acumulado` incrementam se `vinculo_regime`/`estado_funcional` contam como contribuição; `duracao_incapacidade_meses += 1` se `incapacidade_ocorreu = true` e ainda sem constatação | sempre disponível, exceto se `estado_terminal ≠ null`                                                                                               |
| acumulação de contribuição/serviço  | incremento adicional (p.ex. averbação de tempo)                                                                                                                                                                                                                          | `estado_funcional = ativo`                                                                                                                          |
| mudança de vínculo ou regime        | `vinculo_regime` transita para outro regime sintético catalogado                                                                                                                                                                                                         | só entre regimes cronologicamente possíveis na data corrente (invariante §6.2)                                                                      |
...
| constatação documentada             | adiciona entrada com `base_avaliacao: constatacao_documentada`, `responsavel`/`data`/`referencia` preenchidos; zera `duracao_incapacidade_meses` como acumulador de "duração sem constatação" (o valor histórico permanece no rastro, §13)                               | `incapacidade_ocorreu = true`                                                                                                                       |
...
- **Vigência normativa dentro do período**: `versao_rol_vigente` e
  `vinculo_regime`, em qualquer instante, só podem assumir valores cuja
  janela de vigência (as mesmas janelas documentadas em
  `okf/dispositivos/`, base normativa §2) cobre `instante_simulado` — ou
  são deliberadamente marcados `pendente` (a estratégia de stress de §10
  para testar o comportamento fail-closed).
...
- **Probabilidades válidas**: toda linha do kernel (a distribuição de
  transições possíveis a partir de um estado) soma exatamente 1 (tolerância
  de ponto flutuante a definir na implementação); nenhuma probabilidade
  negativa.
...
- **geração de contradições deliberadas** — estados onde
  `nexo_acidente_servico = satisfeito` mas `classe_causa = causa_comum`,
  testando se o avaliador reporta a contradição em vez de escolher um lado
  silenciosamente;
...
- **variação da versão do rol** — gerar trajetórias sob cada
  `versao_rol_vigente` conhecida (LCE 432/2008: 14 doenças; LCE 1.100/2021:
  16\) e também sob `pendente`, testando o comportamento fail-closed de RFC
  0004 §5.3;
...
- **trajetórias que atravessam mudança normativa** — um `instante_simulado`
  ...
  estático perto do limite), testando se `versao_rol_vigente`/`vinculo_regime`
  transitam corretamente no momento certo;
...
1. **t₀ = 2010-03** — ingresso no serviço público e no cargo (mesma data,
   sem mudança de cargo nesta trajetória); `vinculo_regime = lc-1100-2021`
   (ingresso já sob o regime vigente hoje); `estado_funcional = ativo`;
   `incapacidade_ocorreu = false`.
...
3. **t₂ = 2023-07** — **ocorrência de incapacidade**:
   `incapacidade_ocorreu: false → true`; `data_ocorrencia_incapacidade = 2023-07`; `classe_causa = indeterminada`; `duracao_incapacidade_meses = 0`; todo `fatos_verificacao_humana[]` relevante começa
   `base_avaliacao: sem_informacao`, `avaliacao: indeterminado`.
...
5. **t₄ = 2024-02** — **perícia / constatação documentada**:
   ...
   anterior de `hipotese_informada` (§4.2).
...
| --- | ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| t₂  | incapacidade ocorrida, causa indeterminada | `0021`/`0022` não excluídas pelos critérios conhecidos (tipo de benefício, datas), sem campo de causa — indistinguíveis; pendência explícita de indistinguibilidade (mesma lógica de `sinalizarDivergenciaEntreRegrasIndistinguiveis`, `simulador.ts`) | **vazio** — nenhum grupo de substituição de invalidez está `estado_grupo: ativo` hoje (RFC 0004 §16) | as duas faces de `origens_legacy: [regra-0022]` (acidente, RFC 0004 §16.1; doença, §16.2), ambas com o requisito "nexo com acidente em serviço" `indeterminado` (`sem_informacao`) |
| t₃  | + hipótese informada de acidente           | inalterado (o legado não tem esse predicado estruturado)                                                                                                                                                                                               | inalterado (vazio)                                                                                   | face acidente passa a `avaliacao: satisfeito`, rotulada **explicitamente** `hipotese_informada`; face doença permanece `indeterminado`                                             |
| t₄  | + constatação documentada                  | inalterado                                                                                                                                                                                                                                             | inalterado (vazio)                                                                                   | face acidente passa a `avaliacao: satisfeito`, rotulada `constatacao_documentada`; o rastro preserva as duas entradas anteriores (§4.2)                                            |
...
perícia sintética produz `{avaliacao: não satisfeito}` (nexo negado). O
universo 3 diverge exatamente nesse ponto: a face acidente passa a
`excluída` (requisito confirmado não satisfeito), enquanto a face doença
(nunca avaliada pela mesma constatação, porque é um predicado distinto)
permanece `indeterminado`. O universo 1 continua **idêntico** em ambos os
ramos do contrafactual — ele não tem o predicado estruturado, então não tem
como reagir à mudança. Essa divergência (universo 3 muda, universo 1 não)
é precisamente o tipo de achado que a métrica "divergência entre
universos" (§8) foi desenhada para capturar, e ilustra por que os três
universos precisam ser comparados lado a lado em vez de reportados
isoladamente.
