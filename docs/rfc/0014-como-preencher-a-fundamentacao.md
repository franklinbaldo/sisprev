...
O exemplo abaixo é **ilustração do formato, não texto autorado no bundle**. A unidade que o inspira, `invalidez-cf88-original-acidente-em-servico`, **não satisfaz o pré-requisito da §2.3** — não vincula `FormaCalculo` e registra como pendente "confirmar a projeção da forma de cálculo no Sisprev". Nenhuma fundamentação do Ciclo 1 está autorada nesta data, e é assim que tem de ser enquanto as fórmulas não fecharem.

Isto vale como advertência sobre a própria RFC: a primeira tentativa de aplicá-la escreveu duas fundamentações que pareciam completas e não podiam existir — uma delas fundando a paridade no dispositivo errado. O formato não protege contra isso; o pré-requisito da §2.3 e a conferência da §2.2 é que protegem. A unidade proporcional do mesmo grupo, `invalidez-cf88-original-causa-comum`, **não** tem exemplo aqui: a medida da proporção na janela da CF/88 original não tem fonte identificada — a Constituição diz "proporcionais" sem denominador e a LC 228/2000 é posterior —, então não há `FormaCalculo` a vincular e a fundamentação não pode ser autorada (§2.3).
