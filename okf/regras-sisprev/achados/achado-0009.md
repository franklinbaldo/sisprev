...
type: Achado
id: achado-0009
nome: 'Pendência de Preenchimento: Regras Integrais Sem Fundamentação Proporcional
  Preenchida'
situacao: aberto
...
As regras citadas estão configuradas com o campo `integral` = N (ou seja, presumivelmente são regras de aposentadoria/pensão com cálculo proporcional ou semelhante), porém a seção `# Fundamentação Proporcional` está vazia.

# Evidências

A detecção `P9_INTEGRAL_SEM_FUNDAMENTACAO` registrou a ausência de conteúdo na seção de fundamentação proporcional para as regras listadas, que declaram `integral: N`.
