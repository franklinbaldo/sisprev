...
from compilador_proposta import (
    compilar,
    detectar_colisoes,
...
from regra_proposta_schema import LEGACY_FRONTMATTER_KEYS, RegraProposta
...
def _unidade_completa(doc_id: str = "invalidez-acidente-pos-2003", **overrides: object) -> RegraProposta:
...
_DISPOSITIVO_IDS = frozenset({"lce-1100-2021/art-30-par-5/original"})
...
def test_requisito_without_carrier_content_fails() -> None:
    ...
    unidade_sem_portador = RegraProposta(doc_id=unidade.doc_id, frontmatter=frontmatter)
...
def test_causa_without_requisito_is_incoerente_and_pending() -> None:
    ...
    unidade_sem_requisito = RegraProposta(doc_id=unidade.doc_id, frontmatter=frontmatter)
...
def test_generic_human_requirement_in_fundamentacao_does_not_require_incapacity_cause() -> None:
    ...
    unidade_exposicao = RegraProposta(doc_id=unidade.doc_id, frontmatter=frontmatter)
...
def test_unit_not_marked_deployable_cannot_compile_in_deployable_mode() -> None:
    """estado_proposta must already be deployable to compile in deployable mode."""
    unidade = _unidade_completa(estado_proposta="preview")
    resultado = compilar(
        unidade, modo="deployable", legacy_regra_ids=_LEGACY_IDS, dispositivo_ids=_DISPOSITIVO_IDS
    )
    assert resultado.deployable is False
