...
`lce-949-2017` dos cinco dispositivos — e é isso que aparece hoje na fila
`redacao_ausente` do `relatorio_citacoes.py` para
`lce-432-2008/art-62/lce-949-2017`.

Essa entrada da fila **não é tarefa de transcrição**: a redação pedida não
existe e não pode ser transcrita. Enquanto o campo não for decidido, esses
cinco vínculos ficam pendentes nas duas regras, e a lacuna permanece
visível no relatório — que é o comportamento correto, e a razão de o leitor
recusar em vez de "consertar" a atribuição sozinho.
