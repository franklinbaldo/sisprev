...
def test_log_records_the_commit_it_was_generated_from(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Gerado até: <sha> makes staleness visible instead of silent (PR #7 review)."""
    bundle_dir = _init_throwaway_repo(tmp_path)
    monkeypatch.setattr(regras_log, "REPO_ROOT", tmp_path)
    git = shutil.which("git")
    assert git is not None
    ...
    regenerate_regras_log(bundle_dir)

    log_text = (bundle_dir / "regras" / "log.md").read_text(encoding="utf-8")
    assert f"Gerado até: {head}" in log_text
