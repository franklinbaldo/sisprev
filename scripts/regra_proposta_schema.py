...
An audited unit (``type: RegraProposta``) lives in a bundle separate from
the legacy catalog (``okf/regras-propostas/``, see ``okf_common.DEFAULT_BUNDLE_PROPOSTO``)
...
Python types; ``RegraPropostaFrontmatter`` is the on-demand semantic
...
class RegraPropostaValidationError(Exception):
...
class RegraPropostaFrontmatter(ConceptFrontmatter):
    """Typed frontmatter contract for ``type: RegraProposta`` (RFC 0004 §3)."""

    type: Literal["RegraProposta"]
    schema_version: Literal[1]
    estado_proposta: Literal["elaboracao", "preview", "deployable"]
...
class RegraProposta(Concept):
    ...
    @cached_property
    def _validation(self) -> RegraPropostaFrontmatter | ValidationError:
        try:
            return RegraPropostaFrontmatter.model_validate(self.frontmatter)
        except ValidationError as exc:
            return exc

    @property
    def contract(self) -> RegraPropostaFrontmatter | None:
        """Return the validated RFC 0004 frontmatter contract, or None if malformed."""
        result = self._validation
        return result if isinstance(result, RegraPropostaFrontmatter) else None
    ...
    def estado_proposta(self) -> str:
        ...
        if self.contract is not None:
            return self.contract.estado_proposta
        return str(self.frontmatter.get("estado_proposta") or "")
...
def load_regras_propostas(bundle_dir: Path) -> list[RegraProposta]:
    """Load every authored audited-unit document, in filename order.

    Returns an empty list if ``regras/`` doesn't exist yet — the loader
    must accept an empty audited bundle (RFC 0004 §1.2).
    """
    unidades_dir = bundle_dir / "regras"
    ...
    for doc_path in sorted(unidades_dir.glob("*.md")):
        ...
        try:
            ...
            raise RegraPropostaValidationError(msg) from exc
        unidades.append(
            RegraProposta(doc_id=doc_path.stem, frontmatter=frontmatter, body=body, bundle_dir=bundle_dir)
        )
    return unidades


def _validate_identity(unidade: RegraProposta, *, known_legacy_ids: frozenset[str]) -> list[Violation]:
    ...
    if UNIDADE_ID_RE.fullmatch(doc_id) is None or LEGACY_REGRA_REF_RE.fullmatch(doc_id) is not None:
        violations.append(
            Violation(
                "PROPOSTA_ID_INVALIDO", f"{doc_id}: id must be kebab-case and must not be a regra-NNNN shape"
            )
        )
    if unidade.frontmatter.get("id") != doc_id:
        violations.append(
            Violation(
                "PROPOSTA_ID_INVALIDO",
    ...
    violations.extend(
        Violation(
            "PROPOSTA_ORIGEM_INEXISTENTE", f"{doc_id}: origens_legacy references unknown legacy rule {ref!r}"
        )
...
def validate_unidade(unidade: RegraProposta, *, known_legacy_ids: frozenset[str]) -> list[Violation]:
    """Return every intra-document and provenance violation for one audited unit."""
    violations: list[Violation] = []
    if unidade.validation_error is not None:
        violations.extend(
            Violation("PROPOSTA_FRONTMATTER_INVALIDA", error)
...
def validate_bundle_proposto(unidades: list[RegraProposta], bundle_legado: Bundle) -> list[Violation]:
    ...
    violations.extend(
        Violation("PROPOSTA_ID_DUPLICADO", f"duplicate audited unit id {dup!r}") for dup in duplicates
    )
