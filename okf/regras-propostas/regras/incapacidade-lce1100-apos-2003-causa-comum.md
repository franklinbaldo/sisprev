...
proporcionalizada em dias pelo art. 26, com reajuste sem paridade do art. 27,
II, apurado em separado). A contradição entre o frontmatter e
`proveniencia.notas` sobre o rótulo projetado foi corrigida (issue #122).
Cotejados e conformes: dispositivos citados, datas da coorte a partir de 2004,
`integral: N` e `paridade: N` conforme a matriz T7 de `ciclo-01.md`, e a
fundamentação nomeia corretamente a exclusão das causas qualificadas. Esta
verificação é insumo para a conferência humana da coordenação, não substituto
dela — ver a caixa correspondente em "Pendências localizadas".
