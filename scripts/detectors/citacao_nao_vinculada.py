...
- ``fragmento`` / ``sem_norma`` / ``endereco_invalido`` — the prose cites
  something the schema cannot address, or names no owning norm. These need a
  human reading, and saying so is the point: rounding them to the nearest
  provision would claim more than the prose did.
...
def detect(bundle: Bundle) -> list[Detection]:
    ...
    autorados = {doc_id.rsplit("/", 1)[0] for doc_id in bundle.dispositivo_ids()}
    ...
    for regra in bundle.regras:
        ...
        for citacao in citacoes:
            ...
            if endereco is None or endereco in declarados:
                continue
            if endereco in autorados:
                nao_vinculadas.add(endereco)
            else:
                sem_dispositivo.add(endereco)

        if not (nao_vinculadas or sem_dispositivo or nao_enderecaveis):
            continue
