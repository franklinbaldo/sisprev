...
| C1-R31  | C1-R30         | Média sem proporcionalização pelo tempo de contribuição                            | LCE 1.100/2021, art. 24 (base) + art. 30, § 13 (remissão)                                                  | 38 regras (acidente, moléstia profissional, 17 doenças × 2 coortes)    | `tipo_calculo: Valor Médio`, `integral: S`                                                                           | programático                                                           | compilador / agente                                          | conferir `tipo_calculo`/`integral` para toda regra de causa qualificada                                                                          | leitura de campo                                                                                                                                    | cálculo               | coberto                                       | `tipo-calculo-media-80-contribuicoes-lce1100.md`, fidelidade parcial (detalhe operacional, não a base)                                                                                                                                                                                       |
| C1-R32  | C1-R30         | Média proporcionalizada em dias — derivação jurídica                               | LCE 1.100/2021, art. 26 (fração) + art. 30, § 14 (remissão)                                                | 2 regras `*-causa-comum.md`                                            | `tipo_calculo: Proporcionalidade Dias`, `integral: N`                                                                | programático (derivação) / dependência externa (mapeamento no Sisprev) | compilador (derivação) / IPERON e fornecedor (mapeamento)    | fórmula jurídica integralmente determinada em `tipo-calculo-media-proporcional-dias-lce1100.md`: média do art. 24 proporcionalizada pelo art. 26 | leitura de campo (derivação); confirmação do IPERON/fornecedor de que `Proporcionalidade Dias` identifica esta fórmula sem ambiguidade (mapeamento) | cálculo / implantação | **derivação: coberto; implantação: pendente** | `estado_auditoria: concluida`, `estado_implantacao: pendente_mapeamento_sisprev` — não bloqueia a derivação nem o fechamento do ciclo (RFC 0004 round 9); issue #122                                                                                                                         |
...
| Causa comum                    | —      | art. 30, caput e § 14 | `incapacidade-lce1100-ate-2003-causa-comum`                          | `incapacidade-lce1100-apos-2003-causa-comum`                          | R20,R25,R30,**R32**,R42 + comuns                                                 | concluida / concluida | **derivação concluída; implantação pendente (C1-R32)** |
...
| ----------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------- | ---------- |
| Confirmação de que `Proporcionalidade Dias` identifica esta fórmula no Sisprev sem ambiguidade com outras fórmulas de causa comum que compartilham o rótulo | dependência de implantação (não bloqueia `estado_auditoria: concluida`; bloqueia entrada do componente em `data/regras-propostas.csv`) | C1-R32 (2 regras)                     | #122, #124 |
...
observável no catálogo, não suposição. Falta confirmação de que o rótulo —
ou outro campo do schema
legado — identifica esta fórmula sem ambiguidade material perante as
demais. Essa confirmação é dependência de implantação
(`estado_implantacao: pendente_mapeamento_sisprev`), não pendência de
derivação: as duas regras são `estado_auditoria: concluida`. O que ela
impede é especificamente a entrada do componente de implantação a que
pertencem em `data/regras-propostas.csv` (`okf/spec/regraproposta.md`,
"Atomicidade é derivada, não declarada") — só as duas origens de causa
comum (`regra-0020`, `regra-0021`) permanecem a fonte operacional de
exportação até a confirmação; as trinta e oito demais regras propostas do
Bloco C, que não compartilham origem com a causa comum, já entram na
carga, substituindo `regra-0019` e `regra-0022` por inteiro — sem que isso
impeça o fechamento do ciclo quanto a essa derivação (`okf/spec/ciclo.md`,
emenda do round 9 da RFC 0004).
...
correspondentes são `estado_auditoria: concluida`. O que permanece é
`estado_implantacao: pendente_mapeamento_sisprev` — confirmação de que
`Proporcionalidade Dias` identifica esta fórmula sem ambiguidade perante
outras fórmulas de causa comum que compartilham o rótulo no catálogo legado
...
Ela bloqueia, especificamente, a entrada dos dois componentes de
implantação de causa comum do Bloco C — um por coorte, um destino cada —
em `data/regras-propostas.csv`. A decisão jurídica de que os vinte
destinos de cada coorte, juntos, cobrem exaustivamente as causas do
art. 30 está registrada e não foi revista desde 03/08/2026
(`okf/regras-sisprev/ciclos/ciclo-01.md`, T7) — mas é o grafo
origem↔destino, não essa decisão, que determina os componentes de
implantação: `regra-0019`/`regra-0022` (dezenove destinos cada, sem
origem compartilhada com a causa comum) já entram na carga; só
`regra-0020`/`regra-0021` (um destino cada, o de causa comum) permanecem
fora, porque a entrada de cada componente depende também do estado de
implantação dos seus próprios destinos (RFC 0004, round 11;
`okf/spec/regraproposta.md`), não do coorte inteiro.
...
| mapeamento pendente de `tipo_calculo` (C1-R32) | `okf/tipos-calculo/tipo-calculo-media-proporcional-dias-lce1100.md`, [issue #122](https://github.com/franklinbaldo/sisprev/issues/122)                |
