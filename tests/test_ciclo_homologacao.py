...
from unidade_auditada_schema import UnidadeAuditada, load_unidades_auditadas
...
@pytest.fixture(scope="module")
def unidades() -> list[UnidadeAuditada]:
    """As unidades auditadas comitadas — os destinos dos grupos."""
    return load_unidades_auditadas(DEFAULT_BUNDLE_AUDITADO)
...
def test_conjunto_sem_grupo_na_cadeia_levanta(bundle: Bundle, unidades: list[UnidadeAuditada]) -> None:
...
def test_toda_origem_e_destino_do_ciclo_aparece_na_projecao(
    bundle: Bundle, unidades: list[UnidadeAuditada]
) -> None:
    """Cada destino declarado vira exatamente uma linha, e as origens viajam com ela."""
    por_id = {conjunto.doc_id: conjunto for conjunto in bundle.conjuntos}
    grupos = grupos_do_conjunto(CONJUNTO_DO_CICLO_1, por_id)
    destinos = [ref for grupo in grupos for ref in grupo.destinos_auditados]
...
def test_a_linha_sai_com_os_cabecalhos_do_sisprev(bundle: Bundle, unidades: list[UnidadeAuditada]) -> None:
...
def test_o_csv_comitado_bate_com_o_bundle_vivo(bundle: Bundle, tmp_path: Path) -> None:
    ...
    emitidos = regenerate_homologacao(bundle, DEFAULT_BUNDLE_AUDITADO, tmp_path)
...
def test_regenerate_e_no_op_sem_bundle_auditado(bundle: Bundle, tmp_path: Path) -> None:
    """Um bundle sem `okf/regras-auditadas/` ao lado não quebra a derivação."""
