...
``verificar()`` are pure functions over already-loaded ``RegraProposta``
...
- **deployable** — fail-closed: any operational pendency, missing role,
  ...
  ``estado_proposta`` must also already be ``"deployable"``.
...
def _projetar_linha(contrato: RegraPropostaFrontmatter) -> dict[str, str]:
...
def _checar_origens(
    contrato: RegraPropostaFrontmatter, unidade_id: str, *, legacy_regra_ids: frozenset[str]
) -> list[Violation]:
    return [
        Violation(
...
def _checar_portadores(
    contrato: RegraPropostaFrontmatter, unidade_id: str, linha: dict[str, str]
) -> list[Violation]:
    return [
        Violation(
...
def _checar_temporalidade(contrato: RegraPropostaFrontmatter, unidade_id: str) -> list[Violation]:
...
def _checar_proveniencia(
    contrato: RegraPropostaFrontmatter, unidade_id: str, *, dispositivo_ids: frozenset[str]
) -> list[Violation]:
    pendencias = [
        Violation(
...
def _checar_pendencias(
    contrato: RegraPropostaFrontmatter,
    unidade_id: str,
    linha: dict[str, str],
    *,
...
def compilar(
    ...
    *,
    ...
    if contrato.estado_proposta != "deployable":
        pendencias = (
            *pendencias,
            Violation(
                "P_COMPILA_ESTADO_INVALIDO",
                f"{unidade.doc_id}: estado_proposta={contrato.estado_proposta!r}, exige 'deployable'",
            ),
        )
    if pendencias:
        return ResultadoCompilacao(
...
def verificar(unidade: RegraProposta, valores_esperados: dict[str, str]) -> ResultadoVerificacao:
