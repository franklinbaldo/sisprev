...
| C1-R32  | C1-R30         | Média proporcionalizada em dias                                                    | LCE 1.100/2021, art. 26 (fração) + art. 30, § 14 (remissão)                                                | 2 regras `*-causa-comum.md`                                            | `tipo_calculo: Proporcionalidade Dias`, `integral: N`                                                                | programático (projeção) / dependência externa (execução) | compilador (projeção) / IPERON e fornecedor (execução)       | fórmula jurídica confirmada em `forma-calculo-media-proporcional-dias-lce1100.md`; execução no Sisprev não confirmada | confirmação do IPERON/fornecedor de que o rótulo executa a fórmula composta                               | cálculo / implantação | **pendente (dependência de implantação)** | fidelidade parcial severa — omite a própria base média, RFC 0004 §5.3; `estado_proposta: preview`, issue #122            |
...
| Confirmação de que `Proporcionalidade Dias` executa a fórmula composta     | dependência de implantação (bloqueia `deployable`)   | C1-R32 (2 regras)                     | #122, #124 |
...
- **C1-R32** — a confirmação de que `Proporcionalidade Dias` executa a
  fórmula composta, nas duas regras de causa comum. É dependência de
  implantação: bloqueia `deployable` por RFC 0004 §5.3 enquanto não
  confirmada, e por isso essas duas regras e os dois grupos de substituição
  do Bloco C permanecem, respectivamente, `preview` e `estado_grupo: inativo`.
