...
- **ALE-RO/SAPL** (`sapl.al.ro.leg.br`) para a ECE 146/2021 — o PDF é
  digitalizado/assinado (sem texto extraível). O endpoint ao vivo
  respondeu **HTTP 429 em todas as tentativas desta pesquisa** (7
  tentativas, em dois dias de trabalho distintos, incluindo 6 novas
  tentativas espaçadas por 20s nesta rodada) — bloqueio do WAF ao IP do
  ambiente de execução, não uma falha pontual; **não** foi possível obter
  o PDF ao vivo neste ambiente. A transcrição do art. 4º foi, ainda assim,
  **re-verificada de forma independente nesta rodada**: novo download da
  cópia arquivada (Wayback Machine) do mesmo URL oficial, nova extração
  por OCR (`tesseract`, `-l por`) refeita do zero (sem reaproveitar a
  transcrição anterior), cotejada palavra por palavra contra a rodada
  anterior e contra o layout/numeração dos artigos vizinhos (3º, 5º, 6º) —
  identidade confirmada, sem divergência. Por isso a confiança sobe de
  MEDIUM para **HIGH quanto ao texto do art. 4º**; a pendência P-EC146
  fecha nesse ponto específico (§6). Um detalhe adicional, não pedido mas
  encontrado ao ler o art. 13 do próprio documento: fica registrado à
  parte, não eleva nem derruba a confiança do texto (ver nota de vigência
  abaixo).
...
| `ece-146-2021/art-4`         | ECE 146/2021, art. 4º (caput + § único)           | 2021-09-09 →¹               | ALE-RO/SAPL (via Wayback, texto re-verificado) | **HIGH**¹ |
...
¹ **Sobre `ece-146-2021/art-4`**: a confiança HIGH cobre o **texto**, agora
re-verificado nesta rodada por um novo download da mesma cópia arquivada
(Wayback Machine) do PDF oficial ALE-RO e uma nova extração por OCR feita
do zero (`tesseract -l por`, ver §1), palavra por palavra idêntica ao
transcrito. A data `2021-09-09` que aparece na coluna de vigência é a de
**promulgação**, lida no rodapé do próprio documento ("ASSEMBLEIA
LEGISLATIVA, 9 de setembro de 2021"). O art. 13 da própria emenda diz que
ela "entra em vigor **na data de sua publicação**" — não da promulgação —
e esta pesquisa **não confirmou** a data de publicação (DOE/DOM) contra o
Diário Oficial primário; um agregador secundário não oficial (LegisJet)
indica 14/09/2021, cinco dias depois. Fica registrado como nota, não como
pendência bloqueante: nenhuma das 11 regras usa uma data fina o bastante
para essa diferença de 5 dias mudar um enquadramento (`data_adm_ate`/
`data_direito_ate` das regras em questão são todas de anos-calendário).
...
| P-EC146 | `ece-146-2021/art-4`: o **texto** foi re-verificado nesta rodada por novo OCR independente do PDF oficial ALE-RO (confiança HIGH; endpoint ao vivo do SAPL segue respondendo 429 neste ambiente, ver §1) — **fechada** quanto ao texto. Continua aberta, mais restrita, quanto à **data exata de vigência**: o art. 13 da emenda condiciona "entra em vigor" à data de **publicação**, não à de promulgação (2021-09-09, a única confirmada em fonte primária nesta pesquisa); uma fonte secundária não oficial indica publicação em 2021-09-14 | §2                            | qualquer edição que dependa da data exata de vigência deste dispositivo (nenhuma das 11 regras usa granularidade de dias, ver §2) |
