...
1. **`NO-BREAK SPACE` (U+00A0) dentro do nome.** O nome da requerente traz NBSP
   no lugar de um dos espaços entre sobrenomes. Casamento por string literal com
   espaço comum **falha em silêncio** — a primeira tentativa de correção manual
   deste repositório falhou exatamente assim, e só apareceu porque a verificação
   foi refeita em vez de se confiar no "0 ocorrências". Há **987 NBSP** no
   corpus. Padrão de busca tem de usar `\s`, nunca `" "`.

   *(Este item não reproduz nenhum trecho do nome, por decisão deliberada: uma
   versão anterior dele citava parte do nome para ilustrar a posição do NBSP, o
   que reintroduzia identificador parcial no arquivo escrito para documentar a
   remoção. A varredura final pegou — e a tentativa de corrigir por string
   literal falhou de novo, pelo mesmo NBSP.)*
